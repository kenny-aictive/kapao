...
DEMO_BACKUP = {
    'yBZ0Y2t0ceo':
    ['1XqaKI8-hjmbz97UX9bI6lKxTYj73ztmf', 'yBZ0Y2t0ceo_480p.mp4'],
    '2DiQUX11YaY':
    ['1E1azSUE5KXHvCCuFvvM6yUjQDmP3EuSx', '2DiQUX11YaY_720p.mp4'],
    'nrchfeybHmw':
    ['1Q8awNjA6W4gePbWE5cSAu83CwjiwD0_w', 'nrchfeybHmw_480p.mp4']
}
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-p',
        '--video-path',
        default='D:/Downloads/temp_aictive/Videos/s1/1-front.mp4',
        help='path to video file')
    parser.add_argument('--yt-id', default='', help='youtube url id')
    parser.add_argument('--tag',
                        type=int,
                        default=135,
                        help='stream tag, 137=1080p, 136=720p, 135=480p')
    ...
    parser.add_argument('--start', type=int, default=18, help='start time (s)')
    parser.add_argument('--end',
                        type=int,
                        default=-1,
                        help='end time (s), -1 for remainder of video')
    parser.add_argument('--kp-size',
                        type=int,
                        default=2,
                        help='keypoint circle size')
    parser.add_argument('--kp-thick',
                        type=int,
                        default=2,
                        help='keypoint circle thickness')
    parser.add_argument('--line-thick',
                        type=int,
                        default=3,
                        help='line thickness')
    parser.add_argument('--alpha', type=float, default=0.4, help='pose alpha')
    parser.add_argument('--kp-obj',
                        action='store_true',
                        help='plot keypoint objects only')
    parser.add_argument('--csv',
                        action='store_false',
                        help='write results so csv file')
        ...
        if not osp.isfile(video_path):
            try:
                ...
                if video_path == DEMO_BACKUP[args.yt_id][1]:
                    ...
                    gdown.download("https://drive.google.com/uc?id={}".format(
    ...
    for i, (path, img, im0, _) in enumerate(dataset):
        ...
        bboxes, poses, _, _, _ = post_process_batch(data, img, [],
                                                    [[im0.shape[:2]]],
                                                    person_dets, kp_dets)
        ...
        for j, (bbox, pose) in enumerate(zip(bboxes, poses)):
            x1, y1, x2, y2 = bbox
            cv2.rectangle(im0_copy, (int(x1), int(y1)), (int(x2), int(y2)),
                          args.color,
                          thickness=1)
            if args.csv:
                for x, y, c in pose:
            ...
            if args.face:
                for x, y, c in pose[data['kp_face']]:
                    if not args.kp_obj or c:
                        cv2.circle(im0_copy, (int(x), int(y)), args.kp_size,
                                   args.color, args.kp_thick)
        ...
        if not args.gif and args.fps_size:
            cv2.putText(im0,
                        ...
                        args.fps_size, (255, 255, 255),
                        thickness=2 * args.fps_size)

        if args.gif:
            gif_img = cv2.cvtColor(cv2.resize(im0, dsize=tuple(args.gif_size)),
                                   cv2.COLOR_RGB2BGR)
            if args.fps_size:
                cv2.putText(gif_img,
                            '{:.1f} FPS'.format(1 / t),
    ...
    if args.gif:
        print('Saving GIF...')
        with imageio.get_writer(out_path + '.gif', mode="I",
                                fps=fps) as writer:
