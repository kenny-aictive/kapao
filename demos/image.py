...
if __name__ == '__main__':
    ...
    parser.add_argument('-p',
                        '--img-path',
                        default='D:/Downloads/temp_aictive/Videos/pose_images/s1_2-front-10.83.jpg',
                        help='path to image')
    ...
    parser.add_argument('--color-pose',
                        type=int,
                        nargs='+',
                        default=[255, 0, 255],
                        help='pose object color')
    parser.add_argument('--color-kp',
                        type=int,
                        nargs='+',
                        default=[0, 255, 255],
    ...
    dataset = LoadImages(args.img_path,
                         img_size=imgsz,
                         stride=stride,
                         auto=True)
    ...
    if args.bbox:
        bboxes = scale_coords(img.shape[2:], person_dets[0][:, :4],
                              im0.shape[:2]).round().cpu().numpy()
        for x1, y1, x2, y2 in bboxes:
            cv2.rectangle(im0, (int(x1), int(y1)), (int(x2), int(y2)),
                          args.color_pose,
                          thickness=args.line_thick)

    _, poses, _, _, _ = post_process_batch(data, img, [], [[im0.shape[:2]]],
                                           person_dets, kp_dets)
        ...
        for pose in poses:
            if args.face:
                for x, y, c in pose[data['kp_face']]:
                    cv2.circle(im0, (int(x), int(y)), args.kp_size,
                               args.color_pose, args.kp_thick)
            ...
            if data['use_kp_dets']:
                for x, y, c in pose:
                    if c:
                        cv2.circle(im0, (int(x), int(y)), args.kp_size,
                                   args.color_kp, args.kp_thick)

    if args.kp_bbox:
        bboxes = scale_coords(img.shape[2:], kp_dets[0][:, :4],
                              im0.shape[:2]).round().cpu().numpy()
        for x1, y1, x2, y2 in bboxes:
            cv2.rectangle(im0, (int(x1), int(y1)), (int(x2), int(y2)),
                          args.color_kp,
    ...
    filename = '{}_{}'.format(
        osp.splitext(osp.split(args.img_path)[-1])[0],
