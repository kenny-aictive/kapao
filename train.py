...
def train(
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml,
                      ...
                      num_coords=num_coords).to(device)  # create
        exclude = [
            'anchor'
    ...
    compute_loss = ComputeLoss(model, autobalance=False,
                               num_coords=num_coords)  # init loss class
    ...
    for epoch in range(
        ...
        for i, (
            ...
            if RANK in [-1, 0]:
                ...
                callbacks.on_train_batch_end(ni, model, imgs, targets, paths,
                                             plots, opt.sync_bn)
            # end batch ------------------------------------------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                del ckpt
                callbacks.on_model_save(last, epoch, final_epoch, best_fitness,
                                        fi)
