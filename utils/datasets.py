...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self,
        ...
        self.label_files = img2label_paths(
            self.img_files, labels_dir=self.labels_dir)  # labels
        ...
        self.label_files = img2label_paths(
            cache.keys(), labels_dir=self.labels_dir)  # update
        if single_cls:
            ...
            if random.random() < hyp['fliplr']:
                ...
                if nl:
                    ...
                    if self.kp_flip and labels.shape[1] > 5:
                        labels[:, 5::
                               3] = 1 - labels[:, 5::
                                               3]  # flip keypoints in person object
                        keypoints = labels[:, 5:].reshape(nl, -1, 3)
                        keypoints = keypoints[:, self.
                                              kp_flip]  # reorder left / right keypoints
...
def extract_boxes(
        ...
        labels_dir='labels'):  # from utils.datasets import *; extract_boxes()
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in IMG_FORMATS:
            ...
            # labels
            lb_file = Path(
                img2label_paths([str(im_file)], labels_dir=labels_dir)[0])
...
def autosplit(path='../datasets/coco128/images',
    ...
    for i, img in tqdm(zip(indices, files), total=n):
        if not annotated_only or Path(
                img2label_paths(
                    [str(img)],
                    labels_dir='labels_dir')[0]).exists():  # check label
...
def dataset_stats(path='coco128.yaml',
    ...
    def unzip(path):
        ...
        if str(path).endswith('.zip'):  # path is data.zip
            ...
            assert os.system(f'unzip -q {path} -d {path.parent}'
                             ) == 0, f'Error unzipping {path}'
    ...
    for split in 'train', 'val', 'test':
        ...
        dataset = LoadImagesAndLabels(data[split],
                                      labels_dir=labels_dir)  # load dataset
